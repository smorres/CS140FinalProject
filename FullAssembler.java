...
public class FullAssembler extends Assembler {
	...
	public int assemble(String inputFileName, String outputFileName, TreeMap<Integer, String> errors) {
		...
		// note the DATA delimeter is optional, it may be that the program does not load
		// any data into memory
		/*
		 * ADD 8 <---- code SUB 4 <---- code DATA <----- DATA delimeter (everything
		 * before the delimeter must be Code, everything after the delimeter must be
		 * Data) 0 8 <------ data 1 5 <------ data
		 */
		...
		// note a blankLine is only illegal if it is followed by a non-blank line later
		// on.
		...
		try (Scanner input = new Scanner(new File(inputFileName))) {
			...
			while (input.hasNextLine()) {
				...
				// check for errors associated with non blank lines in this if block
					...
					if (line.charAt(0) == ' ' || line.charAt(0) == '\t') {
						errors.put(lineNum, "error: illegal whitespace");
					}
					...
					if (blankLineFound) {
						errors.put(firstBlankLineNum, "error: illegal blank line");
					...
					if (line.trim().toUpperCase().contains("DATA")) {
						...
						if (!line.trim().toUpperCase().equals("DATA")) {
							...
							errors.put(lineNum, "error: 'data' should be uppercase");
							inCode = true;
						}
						// if we are in code, set the boolean inCode to false
						// else, report a duplicate DATA delimeter error
						// (observe the only way inCode could be false is if we already found a line
				...
				// note we add the trim of the line so that we don't have to deal with illegal
				// whitespace
				// we already found and reported that to the user if it occurred in this first
				// pass
			}
			// this is the second pass
			// reset lineNum to zero, we are going to parse from the beginning of the file
			// again.
				...
				if (line.length() > 0) {
					...
					if (Model.OPCODES.containsKey(parts[0].toUpperCase())) {
						...
						// exactly 1 argument
							...
							if (parts.length > 2) {
								errors.put(lineNum, "error: mnemonic has too many args");
							}
							// in an else if, check and report whether the
							// mnemonic has too few arguments (parts.length < 2)
							else if (parts.length < 2) {
								errors.put(lineNum, "error: mnemonic has too few args");
							}
							...
							// is a valid hexadecimal number
								...
								// now, we check that the argument provided to the instruction is a valid
								// hexadecimal number.
								...
								try {
									...
									errors.put(lineNum,
											"Error on line " + lineNum + ": argument is not a hex number: " + parts[1]);
									retVal = lineNum;
								}
							}
						}
					...
					// otherwise report an invalid mnemonic error
						...
						errors.put(lineNum, "error: invalid mnemonic");
					}
			...
			for (String line : data) {
				...
				if (line.length() > 0 && !line.toUpperCase().contains("DATA")) {
					...
					if (parts.length != 2) {
						errors.put(lineNum, "data entry must consist of two numbers");
					}
					// othwerise we need to make sure parts[0] and parts[1] are hexidecimal
					else {
						// using the try catch block as we did above, try to convert parts[0] to a
						// hexadecimal number
						// if it fails, the error message is that the data's memory address in not a hex
						// number
						try {
							Integer.parseInt(parts[0], 16);
						} catch (NumberFormatException e) {
							errors.put(lineNum,
									"Error on line " + lineNum + ": argument is not a hex number: " + parts[0]);
							retVal = lineNum;
						}
		...
		// finally, if we found no errors, then we call the assemble method from our
		// parent to actual assemble the pasm file
